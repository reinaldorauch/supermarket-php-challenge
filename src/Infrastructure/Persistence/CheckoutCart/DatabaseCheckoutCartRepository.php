...
class DatabaseCheckoutCartRepository implements CheckoutCartRepository
{
    ...
    {
        return $this->db->query('SELECT "id", "createdBy", "responsibleUserId" FROM "checkout_cart" WHERE "deletedAt" IS NULL', PDO::FETCH_CLASS, CheckoutCart::class)
            ->fetchAll();
    }

    public function findById(int $id): CheckoutCart
    {
        $stmt = $this->db->prepare('SELECT "id", "createdBy", "responsibleUserId" FROM "checkout_cart" WHERE "deletedAt" IS NULL AND "id" = ?');
    ...
    {
        $stmt = $this->db->prepare('SELECT "id", "createdBy", "responsibleUserId" FROM "checkout_cart" WHERE "deletedAt" IS NULL AND "responsibleUserId" = ?');
    ...
    public function create(int $userId): CheckoutCart
    {
        $stmt = $this->db->prepare(
            'INSERT INTO "checkout_cart" ("createdBy", "responsibleUserId", "createdAt", "updatedAt") 
                VALUES (:userId, :userId, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP) RETURNING "id", "createdBy", "responsibleUserId"',
        );
        $stmt->setFetchMode(PDO::FETCH_CLASS, CheckoutCart::class);
